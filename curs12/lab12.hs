...
-- 3.
tISZERO :: Term
tISZERO = Lambda n (App (App (Var n) (Lambda x tFALSE)) tTRUE)
{-
*Main> reduce $ App tISZERO tZERO 
Lambda "x" (Lambda "y" (Var "x"))

*Main> reduce $ App tISZERO tONE  
Lambda "x" (Lambda "y" (Var "y"))
-}
...
tPRED :: Term
tPRED = Lambda n (Lambda f (Lambda x (App (App (App (Var n) (Lambda g (Lambda h (App (Var h) (App (Var g) (Var f)))))) (Lambda u (Var x))) (Lambda u (Var u)))))
{-
*Main> reduce $ App tPRED tZERO 
Lambda "f" (Lambda "x" (Var "x"))
*Main> reduce $ App tPRED tONE  
Lambda "f" (Lambda "x" (Var "x"))
*Main> reduce $ App tPRED tTWO 
Lambda "f" (Lambda "x" (App (Var "f") (Var "x")))
-}


tSUB :: Term
tSUB = Lambda m (Lambda n (App (App (Var n) tPRED) (Var m)))
{-
*Main> reduce $ App (App tSUB tTHREE) (tONE) 
Lambda "f" (Lambda "x" (App (Var "f") (App (Var "f") (Var "x"))))
-}


tLEQ :: Term
tLEQ = Lambda m (Lambda n (App tISZERO (App (App tSUB (Var m)) (Var n))))
{-
*Main> reduce $ App (App tLEQ tTHREE) tTHREE 
Lambda "x" (Lambda "y" (Var "x"))

*Main> reduce $ App (App tLEQ tTHREE) tTWO   
Lambda "x" (Lambda "y" (Var "y"))

*Main> reduce $ App (App tLEQ tTWO) tTHREE  
Lambda "x" (Lambda "y" (Var "x"))
-}

tEQ :: Term
tEQ = Lambda m (Lambda n (App (App tAND (App (App tLEQ (Var n)) (Var m))) (App (App tLEQ (Var m)) (Var n))))
{-
*Main> reduce $ App (App tEQ tTWO) tTWO 
Lambda "x" (Lambda "y" (Var "x"))

*Main> reduce $ App (App tEQ tTWO) tTHREE 
Lambda "x" (Lambda "y" (Var "y"))
-}
...
-- 4.
tPAIR :: Term
tPAIR = Lambda x (Lambda y (Lambda f (App (App (Var f) (Var x)) (Var y))))
...
tFIRST :: Term
tFIRST = Lambda p (App (Var p) tTRUE)

tSECOND :: Term
tSECOND = Lambda p (App (Var p) tFALSE)
{-
*Main> reduce $ App tFIRST (App (App tPAIR tTWO) tZERO)
Lambda "f" (Lambda "x" (App (Var "f") (App (Var "f") (Var "x"))))

*Main> reduce $ App tSECOND  (App (App tPAIR tTWO) tZERO)
Lambda "f" (Lambda "x" (Var "x"))
-}
...
tCONS :: Term
tCONS = Lambda h (Lambda t (Lambda s (App (App (Var s) (Var h)) (Var t))))

tHEAD :: Term
tHEAD = Lambda l (App (Var l) tTRUE)

tTAIL :: Term
tTAIL = Lambda l (App (Var l) tFALSE)

tISEMPTY :: Term
tISEMPTY = Lambda l (App (Var l) (Lambda h (Lambda t tFALSE)))

list :: Term
list = App (App tCONS tTHREE) (App (App tCONS tTWO) (App (App tCONS tONE) tNIL))


{- 
*Main> reduce $ App tISEMPTY tNIL 
Lambda "x" (Lambda "y" (Var "x"))

*Main> reduce $ App tHEAD list 
Lambda "f" (Lambda "x" (App (Var "f") (App (Var "f") (App (Var "f") (Var "x")))))

Lambda "s" (App (App (Var "s") (Lambda "f" (Lambda "x" (App (Var "f") (App (Var "f") (Var "x")))))) (Lambda "s" (App (App (Var "s") (Lambda "f" (Lambda "x" (App (Var "f") (Var "x"))))) (Lambda "x" (Lambda "x" (Lambda "y" (Var "x")))))))
-}